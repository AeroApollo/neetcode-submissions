class Solution:
    def lengthOfLongestSubstring(self, s: str) -> int:
        ...
        for right in range(len(s)):
            while s[right] in sub: #this while acts like an if
                sub.remove(s[left]) #keep rm letters from the left until duplicate is rm
                left += 1 #after removing left, we don't need to see it again
            sub.add(s[right]) # if s[r] not in set then add it
            longest = max(longest,right-left+1) #+1 bc 0 ind
        return longest